# -*- coding: utf-8 -*-
"""services/dragon/ -- 龙头战法业务层

由 routes/dragon.py 路由瘦身迁移而来 (Stage 2)。候选名单生成与
一键绑定监控池的业务实现统一收编到此, 路由层只做参数绑定。
对外 API 契约不变。
"""
...
from lib.live_simulator import (
    load_strategy_config, load_watch_pool, save_watch_pool,
    SIM_RUNNER,
)
from services.dragon.strategy.dragon_picker import (
    filter_dragon_candidates, _build_mock_today_stocks,
)
from services.dragon.strategy.dragon_backtest import (
    load_status_meta,
    load_sector_panel,
    load_daily_panel,
...
def bind_candidates(codes: List[str]) -> Dict[str, Any]:
    """一键把若干候选股加入 watch_pool, 并把策略绑定到 dragon_picker (热加载)"""
    codes = [_ensure_market_dot(str(c)) for c in codes if str(c).strip()]
    if not codes:
    ...
    msg = SIM_RUNNER.apply_strategy_config(default=default, per_stock=per)
