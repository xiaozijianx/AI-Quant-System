...
class MiniQMTTraderV2:
    ...
    def _check_risk(self, stock_code, volume, price, side=xtconstant.STOCK_BUY):
        ...
        if price > 0:
            ...
            if order_amount > self.max_order_amount:
                ...
                return False
            # 买入时检查 QMT 账户可用资金, 避免本地资金被 max(0, ...) 截断归零
            if side == xtconstant.STOCK_BUY:
                try:
                    asset = self._trader.query_asset(self._account)
                    if asset and asset.cash < order_amount:
                        print(f"[RISK] 可用资金不足: 需 {order_amount:,.2f}, 有 {asset.cash:,.2f}")
                        return False
                except Exception:
                    pass
        return True
    ...
    def buy(self, stock_code, volume, price=0, strategy_name='', remark=''):
        ...
        if not self._check_risk(stock_code, volume, price, side=xtconstant.STOCK_BUY):
            return None
