# -*- coding: utf-8 -*-
"""services/morning/ -- 晨会分析业务层

由 routes/morning.py 路由瘦身迁移而来 (Stage 2)。业务实现
(缓存管理 / 股票详情可解释性 / SSE 工作流编排) 统一收编到此模块,
路由层只做参数解析与调用。对外 API 契约不变。
"""
...
from routes.common import _json_safe as _clean_nan  # 全站统一清洗 (等价原局部实现)
...
def _load_stock_basic(code: str) -> dict:
    """读取股票基本信息"""
    from services.morning.brief.lib.db_config import execute_query
    rows = execute_query(
...
def _calc_perspective(target_code: str,
    ...
    from services.morning.brief.lib.factor_runner import (
...
def get_stock_detail(code: str, top_industries: int = 3, top_concepts: int = 3,
                     sample_per_industry: int = 15, lookback: int = 90) -> dict:
    """
    返回某只推荐股票的详细可解释性信息 (原路由 stock_detail 端点逻辑)。
    """
    from services.morning.brief.lib.rotation_runner import (
        rank_industries_with_phase, get_sector_member_codes,
    )
    from services.morning.brief.lib.concept_rotation_runner import (
...
async def stream_response(top_industries: int = 3,
                         top_concepts: int = 3,
                         top_stocks: int = 5,
                         sample_per_industry: int = 15,
                         lookback: int = 90,
                         package_id: str = ""):
    """SSE 流式触发晨会, 边跑边推中间状态 (原路由 /stream 端点逻辑)."""
