...
def _normalize_stock_code(stock_code: str) -> str:
    """标准化股票代码 — 去掉 .SH/.SZ/.BJ 后缀，便于兼容查询

    索引中可能存储 600875 或 600875.SH，查询时统一按无后缀形式匹配。
    """
    if not stock_code:
        return stock_code
...
def _stock_match(chunk_stock: str, filter_code: str) -> bool:
    """判断 chunk 的股票代码是否与过滤条件匹配（兼容带/不带后缀）"""
    if not chunk_stock:
        return False
    return _normalize_stock_code(chunk_stock) == _normalize_stock_code(filter_code)
    ...
    if os.path.exists(chunks_json_path):
        ...
        if stock_filter:
            chunks_data = [
                ...
                if _stock_match(c.get("metadata", {}).get("stock_code", ""), stock_filter)
            ]
...
def _project_root() -> Path:
    """从脚本路径向上 4 级推算项目根目录（CASE-AI量化系统/）"""
...
def main():
    ...
    parser.add_argument("--stock", default=None, help="按股票代码过滤（如 688981 或 688981.SH），自动兼容带/不带交易所后缀")
    args = parser.parse_args()

    # 相对路径统一解析到项目根目录，避免 agent 工作目录不同导致找不到索引
    index_dir = args.index_dir
    if not Path(index_dir).is_absolute():
        index_dir = str(_project_root() / index_dir)
    ...
    if chunks:
        ...
        if args.stock:
            # 兼容索引中 stock_code 带/不带交易所后缀的情况
            normalized = _normalize_stock_code(args.stock)
            search_kwargs["filter"] = {"stock_code": normalized}
