...
# Windows 终端默认编码可能为 gbk，强制 stdout/stderr 使用 utf-8 避免中文乱码
...
def load_financial_abstract(stock_code: str, data_dir: str) -> pd.DataFrame:
    ...
    if not os.path.exists(csv_path):
        ...
        print(f"[提示] 请先执行: python agent_config/skills/financial-analysis/scripts/fetch_financial_csv.py --stock {stock_code}")
        sys.exit(1)
...
def main():
    ...
    print(f"[开始] 分析 {stock_code} 近 {args.years} 年财务指标")

    df = load_financial_abstract(stock_code, args.data_dir)
    print(f"[加载] 财务摘要共 {len(df)} 行指标")

    metrics = extract_annual_metrics(df, args.years)
    print(f"[分析] 提取到 {len(metrics)} 个核心指标")

    report = generate_analysis(stock_code, metrics)
    print(report)
