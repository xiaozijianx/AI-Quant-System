...
def main():
    ...
    for s in args.stocks.split(","):
        ...
        code = s.strip().upper().replace(".SH", "").replace(".SZ", "").replace(".BJ", "").strip()
        if code != s.strip():
            print(f"[代码标准化] {s.strip()} -> {code}")
    ...
    if missing:
        print(f"\n[提示] 以下股票缺少财务数据，请先获取:")
        for code in missing:
            print(f"  python agent_config/skills/financial-analysis/scripts/fetch_financial_csv.py --stock {code}")
