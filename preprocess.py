...
def extract_pdf_text(pdf_path: str) -> Tuple[str, List[dict]]:
    """
    使用 pdfplumber 逐页提取 PDF 文本，对中文排版支持更好。
    当 pdfplumber 解析失败时，降级使用 PyPDF2 作为备选方案。
    ...
    try:
        ...
        print(f"  [降级] pdfplumber 解析失败({type(e).__name__}), 尝试使用 PyPDF2...")
    ...
    try:
        ...
        print(f"  [错误] PyPDF2 也解析失败({type(e).__name__}): {e}")
        raise
...
def _embed_chunks_batch(texts: List[str], embedding_model: str = "text-embedding-v4") -> list:
    """批量调用百炼 Embedding API，返回向量列表"""
...
def _save_faiss_index(vectorstore, index_dir: str):
    """保存 FAISS 索引到磁盘（兼容 Windows 非 ASCII 路径）"""
    import tempfile
    import shutil
...
def _load_faiss_safe(index_dir: str, embeddings) -> "FAISS":
    """
...
def _save_index_meta(index_dir: str, all_chunks: List[Dict], embedding_model: str):
    """保存 chunks.json 和 index_meta.json"""
    chunks_data = [
    ...
    print(f"  {len(all_chunks)} 个 chunks, {len(doc_ids)} 个文档, "
...
def build_unified_index(
    ...
    """全量构建统一 FAISS 向量索引（--rebuild 或首次创建时使用）"""
    import numpy as np
    import faiss
    from langchain_community.vectorstores import FAISS
    ...
    all_embeddings = _embed_chunks_batch(texts, embedding_model)
    ...
    ids = np.array(range(len(texts)), dtype=np.int64)
    ...
    _save_faiss_index(vectorstore, index_dir)
    _save_index_meta(index_dir, all_chunks, embedding_model)
    return vectorstore
...
def incremental_index_update(
    ...
    """
    增量更新 FAISS 索引：仅嵌入新 chunks 并追加到已有索引，不重建旧数据。
    ...
    # 加载已有 FAISS 索引（兼容 Windows 中文路径，失败时通过临时目录中转）
    fake_embeddings = FakeEmbeddings(size=1024)
    vectorstore = _load_faiss_safe(index_dir, fake_embeddings)

    # 兼容不同索引类型：langchain 默认保存的 IndexFlatL2 不支持 add_with_ids，
    # 需包装为 IndexIDMap 才能增量追加（重建向量后按原顺序重设 id）
    import faiss
    if not isinstance(vectorstore.index, faiss.IndexIDMap):
        old_index = vectorstore.index
        old_vecs = old_index.reconstruct_n(0, old_index.ntotal)
        old_ids = np.arange(old_index.ntotal, dtype=np.int64)
        wrapper = faiss.IndexIDMap(faiss.IndexFlatL2(old_index.d))
        wrapper.add_with_ids(old_vecs, old_ids)
        vectorstore.index = wrapper

    # 嵌入新 chunks
    new_texts = [c["text"] for c in new_chunks]
    print(f"[增量索引] 使用 {embedding_model} 为 {len(new_texts)} 个新 chunks 生成向量...")
    new_embeddings_list = _embed_chunks_batch(new_texts, embedding_model)

    # 追加到 FAISS 索引
    embeddings_np = np.array(new_embeddings_list).astype('float32')
    new_ids = np.array(range(existing_count, existing_count + len(new_texts)), dtype=np.int64)
    vectorstore.index.add_with_ids(embeddings_np, new_ids)

    # 更新 index_to_docstore_id 映射
    for i in range(len(new_texts)):
        faiss_id = existing_count + i
        vectorstore.index_to_docstore_id[faiss_id] = str(faiss_id)

    # 更新 docstore
    for i, chunk in enumerate(new_chunks):
        doc_id = str(existing_count + i)
    ...
    print(f"[增量索引] 完成: {existing_count} + {len(new_chunks)} = {len(all_chunks)} 个 chunks")
...
def main():
    ...
    print(f"\n[统计] 共处理 {success_count} 个新文件, 总计 {len(all_chunks)} 个新 chunks")
    ...
    if args.skip_index:
        ...
        print("\n[提示] 无可索引内容")
    elif index_exists and not args.rebuild:
        # 增量模式: 已有索引，只嵌入新 chunks 并追加
        print(f"\n[增量索引] 仅嵌入 {len(all_chunks)} 个新 chunks，追加到已有索引...")
        incremental_index_update(all_chunks, str(VECTOR_STORE_DIR), args.embedding_model)
    else:
        # 全量重建: 首次创建或 --rebuild，需加载所有旧 chunks 一起重建
        if skip_files:
        ...
        print(f"\n[全量索引] 共 {len(all_chunks)} 个 chunks -> {VECTOR_STORE_DIR}")
