...
本模块从 CASE-B2 板块轮动分析口径对齐, 独立运行, 不依赖 morning_brief。
计算所需数据来自 PostgreSQL 的 trade_sector_daily / trade_stock_status 表。
"""
...
def load_sector_index(sector_name: str, level: int = 2,
    ...
    """从 trade_sector_daily 加载单板块合成指数 K 线."""
...
def load_all_sectors(level: int = 2, end_date: Optional[str] = None,
    ...
    """一次性批量加载全部满足长度要求的板块指数.

    优化点：原来每个板块一次 SQL，N 个板块需要 N+1 次查询；
    现在改为一次批量查询，按 sector_name 分组构造 DataFrame。
    """
    ...
    result = {}
    for sector_name, group in df_all.groupby("sector_name"):
...
def build_market_benchmark(sector_panel: Dict[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    优化点：用字典一次性构造 DataFrame，避免循环插入列导致的碎片化警告。
    """
    if not sector_panel:
        return pd.DataFrame()
    close_dict = {}
...
def calc_strength_indicators(sector_kline: pd.DataFrame,
    ...
    """计算板块强度三因子：MOM_21 / RS_60 / VOL_RATIO."""
