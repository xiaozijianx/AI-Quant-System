...
from .rotation_core import execute_query, rank_industries_with_phase
from .rotation_store import delete_level, save_day

# 状态文件路径
DATA_DIR = Path(__file__).resolve().parent.parent / "data"
STATUS_FILE = DATA_DIR / "sector_rotation_status.json"
...
def _set_running(job_type: str, total: int, message: str = "") -> None:
    ...
    with _state_lock:
        ...
        _stop_requested = False
    print(f"[sector_rotation] 任务开始: {job_type}, 总数 {total}, 消息: {message}", flush=True)
    _write_status()
...
def _set_progress(current_date: str, progress: int, message: str = "") -> None:
    ...
    with _state_lock:
        ...
        if message:
            _message = message
    print(f"[sector_rotation] 进度: {progress}/{_total}, 当前日期 {current_date}, 消息: {message}", flush=True)
    _write_status()
...
def _set_finished(status: str, message: str = "") -> None:
    ...
    print(f"[sector_rotation] 任务结束: {status}, 消息: {message}", flush=True)
    _write_status()
...
def _get_trading_dates(days: int = 20, end_date: Optional[str] = None) -> List[str]:
    """从 trade_sector_daily 查询最近 N 个交易日."""
    params = []
    conditions = []
    if end_date:
...
def _compute_and_save(trade_date: str, level: int = 2, top_n: Optional[int] = None) -> int:
    """计算并写入单个交易日数据."""
    try:
        df = rank_industries_with_phase(
            level=level,
            end_date=trade_date,
            lookback_days=90,
            top_n=top_n,
        ...
        if df.empty:
            print(f"[sector_rotation] {trade_date} 计算结果为空, 跳过", flush=True)
            return 0
        rows = save_day(df, trade_date, level=level)
        print(f"[sector_rotation] {trade_date} 写入 {rows} 条", flush=True)
        return rows
    except Exception as e:
        print(f"[sector_rotation] {trade_date} 计算失败: {e}", flush=True)
        traceback.print_exc()
        raise


def _run_refresh(trade_date: str, level: int = 2) -> None:
    ...
    try:
        ...
        time.sleep(0.05)
        rows = _compute_and_save(trade_date, level=level)
        if _check_stop():
            _set_finished("stopped", "任务已停止")
            return
        ...
        print(f"[sector_rotation] 单日刷新失败: {e}", flush=True)
        traceback.print_exc()
        _set_finished("error", f"计算失败: {e}")


def _run_rebuild(dates: List[str], level: int = 2) -> None:
    """后台线程：批量重建指定日期列表.

    单日失败会记录错误并继续计算后续日期, 不会中断整个任务。
    """
    if not dates:
        _set_finished("error", "未找到板块日线数据, 请先运行板块数据准备")
        return
    ...
    try:
        for i, trade_date in enumerate(dates, start=1):
            ...
            try:
                _compute_and_save(trade_date, level=level)
                _set_progress(trade_date, i, f"{trade_date} 完成 ({i}/{len(dates)})")
            except Exception as e:
                failed_dates.append(trade_date)
                print(f"[sector_rotation] {trade_date} 失败, 继续后续日期: {e}", flush=True)
        ...
        print(f"[sector_rotation] 批量重建失败: {e}", flush=True)
        traceback.print_exc()
        _set_finished("error", f"重建失败: {e}")


def start_refresh(trade_date: Optional[str] = None, level: int = 2) -> dict:
    """启动单日刷新任务."""
    global _running
    with _state_lock:
        if _running:
    ...
    if not trade_date:
        ...
        if not dates:
            _set_finished("error", "未找到板块日线数据")
            return {"ok": False, "error": "未找到板块日线数据"}
        trade_date = dates[-1]

    # 先写入 running 状态, 再启动线程, 确保前端立即能读到状态
    _set_running("refresh", total=1, message=f"正在计算 {trade_date}")
    t = threading.Thread(target=_run_refresh, args=(trade_date, level), daemon=True)
    t.start()
...
def start_rebuild(days: int = 20,
    ...
    if not dates:
        _set_finished("error", "未找到板块日线数据, 请先运行板块数据准备")
        return {"ok": False, "error": "未找到板块日线数据, 请先运行板块数据准备"}

    # 先写入 running 状态, 再启动线程, 确保前端立即能读到状态
    date_range = f"{dates[0]} ~ {dates[-1]}"
    _set_running("rebuild", total=len(dates),
                 message=f"开始重建 {date_range} 共 {len(dates)} 个交易日")
    t = threading.Thread(target=_run_rebuild, args=(dates, level), daemon=True)
    t.start()
