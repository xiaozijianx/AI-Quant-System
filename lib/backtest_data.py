...
数据源 (按优先级):
    1. PostgreSQL trade_stock_daily
       配置在 .env: WUCAI_SQL_HOST/USERNAME/PASSWORD/PORT/DB
    2. xtdata (miniQMT)
       PostgreSQL 不可用时退回; 需要本机装了 xtquant + 启动了 mini
...
def _db_config() -> dict:
    """读 .env 里的 PostgreSQL 配置 -- 必须用 dotenv 提前 load (app.py 已做)"""
    return {
...
def mysql_available() -> bool:
    """快速检查 PostgreSQL 是否可连 (回测页 ping 用)"""
    try:
        import psycopg2
        cfg = _db_config()
...
def _load_from_mysql(stock_code: str,
    ...
    """从 trade_stock_daily 拉日 K (date/open/high/low/close/volume/amount)

    注: amount/turnover_rate 为可选列, 用于因子库 LIQ_20/量比/换手率等计算
    """
    import psycopg2
    ...
    # 加入 amount/turnover_rate, 供因子库页面计算流动性/换手率因子
    sql = f"""
    ...
    # 列重命名: 保留 amount/turnover_rate (因子库需要), 旧回测代码只用前5列不受影响
    df.columns = ["open", "high", "low", "close", "volume", "amount", "turnover_rate"]
    for col in df.columns:
        df[col] = pd.to_numeric(df[col], errors="coerce")
    # 换手率合理性过滤: 正常换手率为百分数(0~100), 超出视为数据异常置 NaN
    # (实测表内存在 150811 等异常值)
...
# ============================================================
# 加载日 K -- xtdata fallback (没有 PostgreSQL 时用)
# ============================================================
...
def _load_from_xtdata(stock_code: str,
    ...
    md = xtdata.get_market_data(
        ...
        start_time=sd,
        end_time=ed,
        # 与主数据源(trade_stock_daily, 前复权)保持口径一致, 避免切换数据源时结果突变
        dividend_type="front",
        fill_data=True,
    )
...
def load_daily_kline(stock_code: str,
    ...
    Args:
        ...
        end_date:    同上
        prefer:      'auto' (PostgreSQL 优先, 失败用 xtdata) / 'mysql' (只用 PostgreSQL) / 'xtdata' (只用 xtdata)
    ...
    if prefer in ("auto", "xtdata"):
        try:
            ...
            last_err = err
    raise RuntimeError(f"加载 {code} 失败 (PostgreSQL + xtdata 都不可用): {last_err}")
...
# ============================================================
# 简单的"股票名"查询 (回测页表头展示, 用 trade_stock_status, 没有就返 code)
# ============================================================

@lru_cache(maxsize=512)
def get_stock_name(stock_code: str) -> str:
    """从 trade_stock_status 取股票中文简称, 取不到返 code"""
    try:
        import psycopg2
        conn = psycopg2.connect(connect_timeout=2, **_db_config())
        try:
            cursor = conn.cursor()
            cursor.execute(
