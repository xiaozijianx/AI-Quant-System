...
- GET  /api/sector-rotation/sector-index  单板块指数 K 线
- GET  /api/sector-rotation/available-dates  表中已有日期
- POST /api/sector-rotation/refresh    刷新某日数据
- POST /api/sector-rotation/rebuild    重建最近 N 日数据
- POST /api/sector-rotation/stop       停止当前任务

实现迁移至 services/rotation/ 统一引擎 (与概念轮动共用), 对外契约不变。
"""
...
from services.rotation.indicator_notes import get_note_sector as get_note
from lib.stock_utils import get_stock_info
from services.rotation.rotation_store import (
    SECTOR_STORE,
    find_sectors,
)
from services.rotation.rotation_worker import SECTOR_WORKER
...
def status():
    """返回当前后台任务状态."""
    return SECTOR_WORKER.get_status()
...
def matrix(days: int = Query(20, ge=1, le=120),
    ...
    df = SECTOR_STORE.load_matrix(days=days, top_n=top_n, level=level, end_date=end_date)
    if df.empty:
        return {
            "dates": [],
...
def detail(sector: str = Query(..., description="板块名称"),
    ...
    """返回某板块某日期的明细 + 近期历史."""
    row = SECTOR_STORE.get_detail(sector, date, level=level)
    if not row:
    ...
    float_fields = [
        ...
        SECTOR.mom_z_col, SECTOR.rs_z_col, "vol_ratio_z",
    ...
    # 历史走势
    hist_df = SECTOR_STORE.get_history(sector, level=level, days=history_days)
    history = []
    ...
    # 相关性最高的 5 只股票
    relevant_stocks = []
    for item in SECTOR_STORE.get_top_relevant_stocks(sector, date, level=level, limit=5):
...
def sector_index(sector: str = Query(..., description="板块名称"),
    ...
    """返回某板块指数近 N 年的日 K 线."""
    df = SECTOR_STORE.get_item_index(sector, level=level, years=years)
    if df.empty:
...
def available_dates(days: int = Query(60, ge=1, le=250),
                    level: int = Query(2, ge=1, le=2)):
    """返回表中已有的最近 N 个交易日."""
    return {"dates": SECTOR_STORE.get_available_dates(level=level, days=days)}
...
def refresh(date: str = Query(None, description="日期 YYYY-MM-DD, 为空则取最新交易日"),
            level: int = Query(2, ge=1, le=2)):
    """启动单日刷新任务."""
    return SECTOR_WORKER.start_refresh(trade_date=date, level=level)
...
def rebuild(days: int = Query(20, ge=1, le=250),
            ...
            end_date: Optional[str] = Query(None, description="结束日期 YYYY-MM-DD, 为空则取最新交易日")):
    """启动批量重建任务."""
    return SECTOR_WORKER.start_rebuild(days=days, level=level, end_date=end_date)


@router.post("/stop")
def stop():
    """停止当前任务."""
    return SECTOR_WORKER.stop_current_job()
