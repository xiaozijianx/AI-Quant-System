...
from concept_rotation.indicator_notes import get_note
...
from concept_rotation.rotation_worker import (
    get_status,
    start_refresh,
    start_rebuild,
    stop_current_job,
...
def matrix(days: int = Query(20, ge=1, le=120),
    ...
    df = load_matrix(days=days, top_n=top_n, end_date=end_date)
    if df.empty:
        return {
            "dates": [],
...
def detail(concept_code: str = Query(..., description="概念编码"),
    ...
    """返回某概念某日期的明细 + 近期历史."""
    row = get_detail(concept_code, date)
    if not row:
    ...
    # 历史走势
    hist_df = get_concept_history(concept_code, days=history_days)
    history = []
    ...
    for item in get_top_relevant_stocks(concept_name, date, limit=5):
...
def concept_index(concept_code: str = Query(..., description="概念编码"),
    ...
    """返回某概念指数近 N 年的日 K 线."""
    df = get_concept_index(concept_code, years=years)
    if df.empty:
...
def available_dates(days: int = Query(60, ge=1, le=250)):
    """返回表中已有的最近 N 个交易日."""
    return {"dates": get_available_dates(days=days)}


@router.post("/refresh")
def refresh(date: str = Query(None, description="日期 YYYY-MM-DD, 为空则取最新交易日")):
    """启动单日刷新任务."""
    return start_refresh(trade_date=date)
...
def rebuild(days: int = Query(20, ge=1, le=250),
            end_date: Optional[str] = Query(None, description="结束日期 YYYY-MM-DD, 为空则取最新交易日")):
    """启动批量重建任务."""
    return start_rebuild(days=days, end_date=end_date)
